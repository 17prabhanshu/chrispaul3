    ...
    try {
      ...
      setNlpResult({
        entities: data.result ? [
          { value: data.result.event_id, type: "EVENT_ID", confidence: 1.0 },
          { value: data.result.status, type: "STATUS", confidence: 1.0 },
          { value: `Anomaly: ${data.result.anomaly_score?.toFixed(3)}`, type: "RISK", confidence: 0.99 }
        ] : [],
        flags: data.result ? ["LIVE INTERCEPT SUCCESS", "DARKINT ML EXECUTED"] : [],
        riskScore: data.result?.anomaly_score ? Math.round(data.result.anomaly_score * 100) : 85,
        decision: "HIGH_CONFIDENCE_FLAG"
      });
      if (autoIngest) setAutoIngestedCount(1);
    } catch (e) {
      console.error(e);
